...
- **从 Steam 导入** — 扫描你已安装的 Steam 游戏，自动检测每款的存档文件夹（内置数据库、Steam 云，或按名称匹配）；找不到的就退回手动选择。
- **游戏图标** — 从 Steam 导入的游戏会显示它的商店图标（或首字母头像）；每款也能自定义图标。
- **排除文件** — 把存档文件夹内不想备份的文件排除（例如 log）；这些文件不会被复制，还原时也不会被动到。
- **共用备份文件夹** — 在「设置」里设一个所有游戏共用的备份位置（每款还能再加自己的子文件夹），或让每款游戏各用自己的绝对路径。
- **存档预览** — 在每份备份旁显示游戏内截图（可设定图片类型）。
- **常驻系统托盘** — 关闭窗口会缩小到系统托盘，后台持续监视。
- **自动更新** — 安装版会自动从 GitHub Releases 更新。
- **五种语言** — English、繁體中文、简体中文、日本語、한국어，可即时切换。
...
或者按侧边栏的 **↓ 从 Steam 导入**：SaveGuard 会扫描你已安装的 Steam 游戏，从内置数据库、Steam 云文件夹或名称匹配自动填入每款的存档文件夹。勾选要保护的游戏即可 —— 找不到的会留空让你自己手动选。

按 **⚙ 设置** 可以设定一个所有游戏共用的**共用备份文件夹** —— 之后每款游戏的备份路径可以是相对子文件夹（或留空），按「套用到所有游戏」就能一键把现有游戏都切过去。语言选择也在这里。
...
- 设定存在 `%APPDATA%\SaveGuard\profiles.json`；备份默认输出在 `%APPDATA%\SaveGuard\Backups`，或你在**设置**里指定的共用文件夹。
